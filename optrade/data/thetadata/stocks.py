...
def get_stock_data_eod(
    ...
    while eod_url is not None:
        ...
        # Then concatenate the date and time_of_day
        eod_df['datetime'] = pd.to_datetime(date + ' ' + time_of_day.astype(str))

        # Remove date and ms_of_day (redundant information)
        eod_df = eod_df.drop(columns=['date', 'ms_of_day', 'ms_of_day2'])
...
if __name__ == "__main__":
    ...
    print(df.head())
    df = get_stock_data_eod(root="AAPL", start_date="20230101", end_date="20230201", clean_up=False, offline=False)
    print(df)
